...
class DataStructureDescriptor(object, metaclass=Singleton):
    ...
    def img_inference(self, root: Path):
    ...
    def img_cls_folder(self, root: Path):
    ...
    def img_anomaly(self, root: Path):
    ...
    def get_num_valid_img_directories(self, root: Path):
        return len([p for p in root.iterdir() if self.valid_img_folder(p)])

    @staticmethod
    def valid_img_folder(root: Path):
        
        def contain_image(root:Path):
